...
class TestBestOfNInitialization:
    ...
    def test_rejects_out_of_range_sigma(self, mock_objective_target, bad_sigma):
        with pytest.raises(ValueError, match="sigma must be between 0.0 and 1.0"):
            BestOfNAttack(objective_target=mock_objective_target, sigma=bad_sigma)

    def test_prepends_augmentation_stack(self, mock_objective_target):
        # The three augmentation converters are prepended in order: scramble, case, noise.
        attack = BestOfNAttack(objective_target=mock_objective_target)
        stack = [cfg.converters[0] for cfg in attack._request_converters[:3]]
        assert isinstance(stack[0], CharSwapConverter)
        assert isinstance(stack[1], RandomCapitalLettersConverter)
        assert isinstance(stack[2], CharNoiseConverter)

    def test_sigma_maps_onto_each_converter_rate(self, mock_objective_target):
        # Scramble and case share sigma ** 0.5; noise is power-scaled far lower at
        # sigma ** 3. Pin all three so a regression to a flat/linear sigma is caught.
        attack = BestOfNAttack(objective_target=mock_objective_target, sigma=0.36)
        scramble = attack._request_converters[0].converters[0]
        case = attack._request_converters[1].converters[0]
        noise = attack._request_converters[2].converters[0]
        assert scramble._word_selection_strategy._proportion == pytest.approx(0.36**0.5)
        assert case.percentage == pytest.approx(0.36**0.5 * 100)
        assert noise.noise_probability == pytest.approx(0.36**3)

    def test_user_request_converter_survives_after_augmentation(self, mock_objective_target):
        ...
        assert isinstance(attack._request_converters[0].converters[0], CharSwapConverter)
        assert isinstance(attack._request_converters[3].converters[0], Base64Converter)
...
class TestBestOfNSampling:
    ...
    async def test_each_attempt_sends_a_freshly_augmented_prompt(self, mock_scorer, basic_context):
        # End-to-end through the real normalizer: the retry loop re-runs the
        # augmentation, so distinct samples reach the target.
        ...
        # More than one distinct prompt means it re-sampled per attempt, not converted once.
